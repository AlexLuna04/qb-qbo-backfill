...
@data_loader
def load_customers_raw(*args, **kwargs):
    """
    Backfill histórico de Customers desde QBO hacia Postgres (raw)
    ...
    print(f"[INIT] Backfill Customers {fecha_inicio} → {fecha_fin}")
    ...
    while current_date < fecha_fin:
        ...
        while True:
            ...
            records = response.get("QueryResponse", {}).get("Customer", [])
            ...
            upsert_raw(
                ...
                meta={
                    ...
                    "request_payload": f"Customers {window_start} → {window_end}"
                }
            )
