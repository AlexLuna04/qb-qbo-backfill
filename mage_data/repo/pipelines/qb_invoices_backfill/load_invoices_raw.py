...
@data_loader
def load_invoices_raw(*args, **kwargs):
    """
    Backfill histórico de Invoices desde QBO hacia Postgres (raw)
    ...
    print(f"[INIT] Backfill Invoices {fecha_inicio} → {fecha_fin}")
    ...
    while current_date < fecha_fin:
        ...
        while True:
            ...
            records = response.get("QueryResponse", {}).get("Invoice", [])
            ...
            upsert_raw(
                ...
                meta={
                    ...
                    "request_payload": f"Invoices {window_start} → {window_end}"
                }
            )
